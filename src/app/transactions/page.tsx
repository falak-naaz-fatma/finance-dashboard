    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <div className="lg:pl-[280px]">
        ...
        <main className="px-4 py-10 sm:px-8 lg:px-10">
          ...
          <section className="overflow-hidden rounded-[8px] border border-white/10 bg-card shadow-[0_24px_70px_rgba(0,0,0,0.32)]">
            <div className="border-b border-white/10 p-4">
              <div className="flex flex-col gap-3 xl:flex-row xl:items-start">
                <div className="min-w-0 flex-1">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="h-9 w-full rounded-[11px] border border-white/10 bg-muted px-10 text-sm font-normal text-foreground outline-none placeholder:text-zinc-400 focus:border-[#8b5cf6]/70 focus:ring-4 focus:ring-[#8b5cf6]/10"
                    />
                  </div>
                  <div className="mt-3 flex flex-wrap gap-2">
                    {monthChips.map((month) => (
                      <button
                        ...
                        className={`h-7 rounded-full px-3 text-xs font-medium transition ${monthFilter === month.value
                ...
                <div className="grid h-8 shrink-0 grid-cols-3 rounded-[10px] bg-muted p-0.5">
                  {(["all", "income", "expense"] as const).map((type) => (
                    <button
                      ...
                      className={`min-w-16 rounded-[8px] px-2.5 text-xs font-medium capitalize transition ${typeFilter === type ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"
                        }`}
                    >
                      {type}
                    </button>
                  ...
                  {loading ? (
                    ...
                    visibleTransactions.map((transaction) => {
                      ...
                      return (
                        <tr
                          key={transaction._id}
                          className="group border-b border-border text-sm text-muted-foreground transition hover:bg-accent"
                        >
                          ...
                          <td className="px-6 py-4">
                            <span
                              className={`inline-flex h-5 items-center rounded-full border px-2.5 text-xs font-semibold uppercase ${isIncome
                          ...
                          <td
                            className={`px-6 py-4 text-right text-sm font-semibold ${isIncome ? "text-[#22d3a6]" : "text-[#ff416d]"
                              }`}
                          >
            ...
            <div className="flex items-center justify-between px-8 py-5">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  disabled={page === 1}
                  className="h-9 rounded-[12px] border border-border bg-background px-4 text-sm font-semibold text-foreground transition hover:bg-accent disabled:cursor-not-allowed disabled:opacity-45"
                >
                  Previous
                </button>
                <button
